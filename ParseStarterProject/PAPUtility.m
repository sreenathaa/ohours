            ...
            if (succeeded && ![[[photo objectForKey:kPAPPhotoUserKey] objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
                ...
                if (privateChannelName && privateChannelName.length != 0) {
                    NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:
                                          [NSString stringWithFormat:@"%@ liked your event", [PAPUtility firstNameForDisplayName:[[PFUser currentUser] objectForKey:kPAPUserUserNameKey]]], kAPNSAlertKey, kAPNSSoundTypeKey, kAPNSSoundKey,
                                          ...
                                          [photo objectId], kPAPPushPayloadPhotoObjectIdKey, @"Increment",kAPNSBadgeKey,
                                          nil];
...
+ (void)processFacebookProfilePictureData:(NSData *)newProfilePictureData {
    ...
    UIImage *smallRoundedImage = [image thumbnailImage:64 transparentBorder:0 cornerRadius:0 interpolationQuality:kCGInterpolationLow]; //cornerRadius:9
...
//    NSString *privateChannelName = [user objectForKey:kPAPUserPrivateChannelKey];  //kPAPUserPrivateChannelKey
//    if (privateChannelName && privateChannelName.length != 0) {
//        NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:
//                              [NSString stringWithFormat:@"%@ is now following you", [PAPUtility firstNameForDisplayName:[[PFUser currentUser] objectForKey:kPAPUserUserNameKey]]], kAPNSAlertKey, kAPNSSoundTypeKey, kAPNSSoundKey, 
//                              kPAPPushPayloadPayloadTypeActivityKey, kPAPPushPayloadPayloadTypeKey,
//                              kPAPPushPayloadActivityFollowKey, kPAPPushPayloadActivityTypeKey,
//                              [user objectId], kPAPPushPayloadFromUserObjectIdKey, @"Increment",kAPNSBadgeKey, 
//                              nil];
//        PFPush *push = [[PFPush alloc] init];
//        [push setChannel:privateChannelName];  
//        [push setData:data];
...
//    }
    ...
    if (privateChannelName && privateChannelName.length != 0) {
        ...
        NSString *string = [NSString stringWithFormat:@"%@ is now following you", [PFUser currentUser].username];
...
//This is no longer being called at all... This function does the same as the one above but without the notification activity
