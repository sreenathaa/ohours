...
- (void)setFlashUser:(PFUser *)mrFlash {
    NSString *key = kPAPUserDefaultsCacheFlashUserKey;
    [self.cache setObject:mrFlash forKey:key];
    [[NSUserDefaults standardUserDefaults] setObject:mrFlash forKey:key];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (PFUser *)mrFlash {
    ...
    PFUser *fu = [[NSUserDefaults standardUserDefaults] objectForKey:key];
    
    if (fu) {
        [self.cache setObject:fu forKey:key];
    }
